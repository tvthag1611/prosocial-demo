...
import * as usersActions from '../../../redux/action-creators/home'
...
export default function CreateGroup() {
  ...
  // const suggestion = users.map((user) => {
  //   return {
  //     id: user.id,
  ...
  //   setTags(tags.filter((tag, index) => index !== index))
  // }
  ...
  return (
    <div className="create-group">
      <div className="write-info-group">
        ...
        <div className="write-info-group__content">
          ...
          <div className="import-cover">
            ...
            <input type="file" className="import-file-cover" id="importCover" />
          </div>
      ...
      <div className="preview-new-group">
        <h5>Preview</h5>
        <img src={group.cover} className="cover-group" alt="cover group" />
          ...
          <div className="header-group__invite">
            {group.members.length <= 5
              ? group.members.map((member, index) => {
                  if (index <= 4) {
                    return <img src={member.avatar} key={index} />
                  }
                })
              : group.members.map((member, index) => {
                  if (index <= 3) {
                  ...
                  if (index == 4) {
                    return (
                      <div className="invited-group__mores" key={index}>
                        ...
                        />
                        <i
                          className="fa fa-ellipsis-h invited-group__more"
                          aria-hidden="true"
                        ></i>
                      </div>
                    )
        ...
        <p className="description-this-group">{group?.description}</p>
      </div>
    </div>
  )
}
